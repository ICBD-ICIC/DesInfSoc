# Calculates amount and ratio of valence positive words using distance words embeddings with a dictionary
...
def valence_positive_words(text, dictionary):
    positive_or_similar = []
    for word in text:
        found = False
        if word in already_found_words:
            positive_or_similar.append(word)
            found = True
    ...
    return " ".join(positive_or_similar)
...
OUTPUT_FILE = '../outputs/valence_positive_{0}.csv'.format(time.time())
...
positive_embeddings = [ft.get_word_vector(word).reshape(1, -1) for word in positive_words]

all_tweets['positive_words'] = all_tweets['clean_text'].apply(lambda text: valence_positive_words(text, positive_embeddings))
all_tweets['positive_words_n'] = all_tweets['positive_words'].str.split().map(len)
all_tweets['positive_words_ratio'] = all_tweets['positive_words_n'].astype('int') / all_tweets['clean_text'].map(len)
